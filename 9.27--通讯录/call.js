window.addEventListener('load',function(){
	...
	function render(data){
		...
		input.onkeyup=function(){
			...
			dl.innerHTML = '';
			data.forEach(function(element){
			if(element.name.includes(values)||element.tel.includes(values)||element.py.includes(values)){
				dl.innerHTML += `<dd><a href="tel:${element.tel}">${element.name}${element.tel}</a></dd>`
				return;
			}
		})
